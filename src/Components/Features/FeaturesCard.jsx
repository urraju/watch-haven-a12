...
const FeaturesCard = ({ data, refetch }) => {
  ...
  const handleVote = () => {
    axiosPublic.patch(`/watch/update?id=${_id}`).then((res) => {
      ...
      <div className="bg-gradient-to-bl relative border rounded-xl from-gray-200  to-yellow-50 justify-between p-4 h-[370px] ">
        ...
        <div className="absolute bottom-0 py-8">
          <div className=" mt-5 ">
            <Link
              to={`/details/${_id}`}
              className="text-lg   flex font-kdam items-center gap-2 text-black mb-2"
            >
              {product_name}
              <FaArrowRight className="text-sm text-yellow-400" />
            </Link>
            <div className="">
              {tags.map((tag) => (
                <button className="  rounded-full border mr-3 px-2 mt-1 text-black bg-gradient-to-br  text-sm ">
                  {tag}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-5 mt-2 items-center">
            {user ? (
              ...
              >
                {" "}
                <FaArrowUp className="text-md font-light text-yellow-500" />
              </button>
            ) : (
              <button
                disabled
              ...
              >
                {" "}
                vote{" "}
                <HiOutlinePlusSm className="text-md font-light text-yellow-500" />
              </button>
            )}

            <button className="flex  items-center gap-2 border px-3 bg-yellow-300 rounded-full text-black text-">
              <FaVoteYea />
              {vote}
            </button>
          </div>
