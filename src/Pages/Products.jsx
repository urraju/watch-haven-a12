...
const Products = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosPublic.get(
        `/watch?search=${search}&page=${currentPage}&size=${perPage}`
      );
      setData(res.data);
      return res.data;
    },
  ...
  return (
    <div className="">
      <HelmetUse helmet={'Products || Page'}/>
      ...
      <div className="flex items-center border-l-2 border-yellow-500  rounded-r mt-10 h-max w-max mx-auto bg-gray-200 justify-center">
        ...
        <form onSubmit={handleSearch} action="">
          <input
            ...
            className="bg-gray-200 border py-2 outline-none px-3 "
            name="search"
            type="text"
          />
          <input
            className="bg-yellow-400 cursor-pointer text-gray-600  uppercase rounded-r px-3 py-2"
            type="submit"
            value="Search"
          />
        </form>
